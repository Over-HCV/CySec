...
// Las operaciones que necesitan una construcción concreta —una opción sin
// marcar, una lista de fuentes, un vecino conocido— trabajan sobre un taller
// escrito para eso. Sobre el de verdad se prueba lo que no depende de lo que
// alguien haya contestado hoy: que la partición sigue cubriendo el archivo y
// que dos personas no se pisan.
const muestra = SAMPLE_TEX
...
describe.skipIf(!hasRepo)('applyFieldEdit con guard', () => {
  /**
...
describe.skipIf(!hasRepo)('applyFieldEdit', () => {
...
describe.skipIf(!hasRepo)('convergencia con edición simultánea', () => {
...
describe.skipIf(!hasRepo)('operaciones de bloque', () => {
  ...
  it('moveBlock intercambia con el vecino visible, saltando los huecos', () => {
    ...
    moveBlock(ytext, blocks, first, 1, muestra)
  ...
  it('toggleOption marca y desmarca una opción', () => {
    const { ytext } = docWith(muestra)
    const opcion = parseDoc(muestra, 'tex').find(b => b.kind === 'mcq')!.items!
      .find(b => b.kind === 'opcion')!
    toggleOption(ytext, opcion, muestra)
  ...
  it('moveBlock ordena también entre hermanos anidados', () => {
    const { ytext } = docWith(muestra)
    const fuentes = parseDoc(muestra, 'tex').find(b => b.kind === 'fuentes')!
    const items = fuentes.items!
    const primera = items.findIndex(b => b.kind === 'fuente')
    moveBlock(ytext, items, primera, 1, muestra)
    ...
    expect(urls[0]).toContain('segunda.example')
    expect(urls[1]).toContain('ejemplo.org')
  })
})

describe('el documento cambió bajo los pies (regresión: ws-01 descolocado)', () => {
  // Un raíz escrito para esto: la regresión iba de rangos caducados, no del
  // contenido del taller, y con el archivo real las cuentas cambiaban cada vez
  // que se añadía una sección.
  const main = SAMPLE_MAIN
...
describe.skipIf(!hasRepo)('contenedores', () => {
