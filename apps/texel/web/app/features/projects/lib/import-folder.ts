...
export function planImport(entries: { relativePath: string, file: File }[]): ImportPlan {
  ...
  for (const entry of entries) {
    ...
    const reason = skipReason(path, entry.file.size)
 ...
 *
 * Se exporta porque la sincronización con GitHub necesita exactamente las
 * mismas reglas —lo que no vale la pena importar de una carpeta tampoco vale la
 * pena traerse de un repo— y ahí no hay `File`, solo rutas y tamaños.
 */
export function skipReason(path: string, size = 0): string | null {
  ...
  if (size > MAX_FILE_BYTES) return `pasa de ${Math.round(MAX_FILE_BYTES / 1024 / 1024)} MB`
