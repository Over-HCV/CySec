...
interface LayoutState {
  ...
  compileMode: StickyCompileMode // profundidad de la compilación
}
 ...
 *
 * `full` tira la caché de auxiliares y rehace el documento entero, bibliografía
 * incluida. Es la salida cuando las referencias se quedan atrás.
 */
export type CompileMode = 'normal' | 'fast' | 'full'

/**
 * Los que se recuerdan entre sesiones. `full` no: es una acción de una vez
 * («Recompilar desde cero»), no una preferencia — dejarlo pegado significaría
 * pagar una compilación completa en cada tecla.
 */
export type StickyCompileMode = Exclude<CompileMode, 'full'>
