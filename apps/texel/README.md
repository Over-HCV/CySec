...
de 1100 px sin perfil ICC ni canal alfa (`app/shared/lib/image-proxy.ts`), y solo
...
y un PDF de 13 MB que subir y volver a bajar; con las derivadas, 1,4 s y 3 MB. Las
...
| | antes | ahora |
|---|---|---|
| rápido, una sección tocada | ~48 s | ~5 s |
| de eso, la pasada de LaTeX | 28,7 s | 3,8 s (2,4 xelatex + 1,4 xdvipdfmx) |
| PDF que se sube y se baja | 12,8 MB | 3,2 MB |
| tarball de auxiliares | 12,9 MB | 162 KB |
...
Y dos cosas que no se ven en la tabla porque no están en el servidor: el visor
conserva la página y el desplazamiento al repintar (antes cada compilación te
devolvía a la primera página), y una compilación que no cambia nada ya no
vuelve a firmar la URL del PDF, así que el navegador no se baja otra vez los
mismos megas para enseñar exactamente lo mismo.

**Probado y descartado:** precompilar el preámbulo con `mylatexformat` (volcar
un `.fmt` con fontspec, unicode-math y tcolorbox ya cargados). El volcado
funciona y la pasada baja de 2,4 s a 0,3 s, pero al leer el `.bbl` xelatex
termina en *segmentation fault*: biblatex y el formato volcado no se llevan
bien bajo XeTeX. Serían ~1,5 s de los 5; no compensa el riesgo.
